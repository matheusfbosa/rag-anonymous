...
| `RAG_ANON_LLM_NUM_PREDICT`      | `512`                       | Max tokens generated per call (`ChatOllama(num_predict=...)`). Prevents unbounded output on adversarial or list-style prompts.                                                                                                  |
...
| `RAG_ANON_LLM_TIMEOUT_SEC`      | `180`                       | HTTP timeout in seconds for each Ollama generation call. On timeout, `query_rag` logs a warning and returns an empty answer so batch runs continue.                                                                              |
